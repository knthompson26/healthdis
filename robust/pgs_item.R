...
# load data
load("/home/thom1336/healthdis/data/dat_pheno_pgs_W2_bin.RData")
load("/home/thom1336/healthdis/data/dat_pheno_pgs_W4_bin.RData")
...
waves <- list(
  W2 = list(data = dat_pheno_pgs_W2_bin, outcome = "CESD_W2"),
  W4 = list(data = dat_pheno_pgs_W4_bin, outcome = "CESD_W4")
)
...
#  dat$UM <- rnorm(nrow(dat))  # mediator error (original)
 ...
 # dat_expanded$UM <- rnorm(nrow(dat_expanded))
  ...
  #----------------------------------------------------
  # Step 1: Variable for BINARY social (M) when pgs_var = 0
  #----------------------------------------------------
  ...
  for (m in mediators) {
    ...
    fitM <- try(                                          # tries to run without crashing whole script
      glm(as.formula(paste(m, "~", pgs_var)),            # build glm formula "mediator ~ pgs"
    ...
    new0[[pgs_var]] <- 0                                 # set exposure = 0 for everyone (counterfactual condition) - make sure calling var [[]]
    ...
    new_var <- paste0(m, "_", pgs_var, "0")             # name of the new simulated mediator variable
  ...
  walk(mediators, function(m) {
    ...
    formula_str <- paste0(
      ...
      "sex + ",                             # c2
      "SES + ",                             # c3
      "black_cov + natam_cov + asian_cov + ",   # c4: race dummies
      "happyP + ",                          # c5
      "drinkP_bin + ",                      # c6
      "health_bin + ",                          # c7
      "violence + ",                        # c8
      ...
      "SES:health_bin + ",
      "SES:violence + ",
      #   "happyP:black_cov + ",  # interactions with the race variables gave null coefs and were dropped
      #   "happyP:natam_cov + ",
      #   "happyP:asian_cov + ",
      "happyP:drinkP_bin + ",
      "happyP:health_bin + ",
    ...
    dat_expanded <<- dat_expanded %>%
      mutate(
        !!output_var_00 :=
          ...
          # X = pgs_var = 0 → all terms multiplied by X drop out
          (coefs[[pgs_var]] %||% 0) * 0 +
          
          # mediator terms
          (coefs[[m]] %||% 0)                       * !!pgs0_var +
          (coefs[[paste0(pgs_var, ":", m)]] %||% 0) * 0 * !!pgs0_var +
          ...
          # interactions with pgs_var (X = 0 → vanish)
          (coefs[[paste0(pgs_var, ":H1GI1Y")]]   %||% 0) * 0 * H1GI1Y +
          (coefs[[paste0(pgs_var, ":sex")]]      %||% 0) * 0 * sex +
          (coefs[[paste0(pgs_var, ":SES")]]      %||% 0) * 0 * SES +
          (coefs[[paste0(pgs_var, ":black_cov")]]%||% 0) * 0 * black_cov +
          (coefs[[paste0(pgs_var, ":natam_cov")]]%||% 0) * 0 * natam_cov +
          (coefs[[paste0(pgs_var, ":asian_cov")]]%||% 0) * 0 * asian_cov +
          (coefs[[paste0(pgs_var, ":happyP")]]   %||% 0) * 0 * happyP +
          (coefs[[paste0(pgs_var, ":drinkP_bin")]]%||% 0) * 0 * drinkP_bin +
          (coefs[[paste0(pgs_var, ":health_bin")]]%||% 0) * 0 * health_bin +
          (coefs[[paste0(pgs_var, ":violence")]]  %||% 0) * 0 * violence +
    ...
    dat_expanded <<- dat_expanded %>%
      mutate(
        !!output_var_10 :=
          ...
          # mediator terms
          (coefs[[m]] %||% 0)                       * !!pgs0_var +
          (coefs[[paste0(pgs_var, ":", m)]] %||% 0) * 1 * !!pgs0_var +
          ...
          # interactions with pgs_var (X = 1)
          (coefs[[paste0(pgs_var, ":H1GI1Y")]]    %||% 0) * 1 * H1GI1Y +
          (coefs[[paste0(pgs_var, ":sex")]]       %||% 0) * 1 * sex +
          (coefs[[paste0(pgs_var, ":SES")]]       %||% 0) * 1 * SES +
          (coefs[[paste0(pgs_var, ":black_cov")]] %||% 0) * 1 * black_cov +
          (coefs[[paste0(pgs_var, ":natam_cov")]] %||% 0) * 1 * natam_cov +
          (coefs[[paste0(pgs_var, ":asian_cov")]] %||% 0) * 1 * asian_cov +
          (coefs[[paste0(pgs_var, ":happyP")]]    %||% 0) * 1 * happyP +
          (coefs[[paste0(pgs_var, ":drinkP_bin")]]%||% 0) * 1 * drinkP_bin +
          (coefs[[paste0(pgs_var, ":health_bin")]]%||% 0) * 1 * health_bin +
          (coefs[[paste0(pgs_var, ":violence")]]  %||% 0) * 1 * violence +
  ...
  model <- lm(
    as.formula(
      paste0(
        outcome_var, " ~ ",
        pgs_var, " + ",
        "H1GI1Y + sex + SES + black_cov + natam_cov + asian_cov + ",
        pgs_var, ":H1GI1Y + ",
        pgs_var, ":sex + ",
        pgs_var, ":SES + ",
        pgs_var, ":black_cov + ",
  ...
  walk(mediators, function(m) {
    me_var <- sym(paste0("me_", pgs_var, "_", m))
  ...
  effects <- map(mediators, function(m) {
    c(
      ME = mean(dat_expanded[[paste0("me_", pgs_var, "_", m)]], na.rm = TRUE),
...
for (wave in names(waves)) {
  for (pgs_var in pgs_vars) {  # nested loop over each pgs variable
    ...
    saveRDS(results, file = paste0("/home/thom1336/healthdis/data/sensitivity/item/confound/pgs/bootstrap_results_ALL_", wave, "_", pgs_var, ".rds"))
