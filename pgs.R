...
load(paste0(data_dir,"dat_pheno_pgs_W2.RData"))  # EDITED LINE
load(paste0(data_dir,"dat_pheno_pgs_W4.RData"))  # EDITED LINE
...
bootstrap_stat <- function(data, indices, wave_name, mediators, pgs_var) {
  ...
  # mitigated effect (me), total causal effect (tce), difference (dif)
  #----------------------------------------------------------------
...
for (wave in names(waves)) {
  for (pgs_var in pgs_vars) {  # nested loop over each pgs variable
    ...
    saveRDS(results, file = paste0(data_dir, "confound/all/pgs/bootstrap_results_ALL_", wave, "_", pgs_var, ".rds"))  # EDITED LINE
