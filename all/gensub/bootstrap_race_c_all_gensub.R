    ...
    dat_expanded <<- dat_expanded %>%  # apply prediction values to all expanded data sets
      mutate(
        !!output_var_00 :=
          ...
          (coefs[[paste0(race_var, ":pgs")]]       %||% 0) * pgs      * 0 +
    ...
    dat_expanded <<- dat_expanded %>%
      mutate(
        !!output_var_10 :=
          ...
          (coefs[[paste0(race_var, ":pgs")]]       %||% 0) * pgs +
  ...
  model <- lm(
    as.formula(
      paste0(
        ...
        race_var, ":sex + ",
        race_var, ":pgs + ",
        "H1GI1Y:SES + H1GI1Y:sex + H1GI1Y:pgs + ",
        "SES:sex + SES:pgs + ",
        "sex:pgs"
      )
    ),
...
for (wave in names(waves)) {
  for (race_var in race_vars) {  # NEW LINE: Nested loop over each race variable
    ...
    saveRDS(results, file = paste0("/home/thom1336/healthdis/data/confound/all/gensub/race/bootstrap_results_", wave, "_", race_var, ".rds"))
  #  saveRDS(boot_results, file = paste0("/home/thom1336/healthdis/data/confound/all/gensub/race/boot_object_", wave, "_", race_var, ".rds"))
