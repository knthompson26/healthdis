 ...
 # dat_expanded$UM <- rnorm(nrow(dat_expanded)) - we dont have a cont mediator anymore so we dont need this
  ...
  walk(mediators, function(m) {
    ...
    formula_str <- paste0(
      ...
      m, " + ", 
      m, ":", ses_var, " + ", 
    #  m, "2 + ",  - no longer needed as binary mediators 
    ...
    dat_expanded <<- dat_expanded %>%  # apply prediction values to all expanded data sets
      mutate(
        !!output_var_10 :=
          ...
          coefs[[m]]             * !!ses0_var +                      # main effect of mediator
          (coefs[[paste0(ses_var, ":", m)]] %||% 0) * !!ses0_var +   # interaction: ses_var × mediator
      #    coefs[[paste0(m, "2")]] * (!!ses0_var)^2 +                # quadratic effect of mediator
  ...
  dat_expanded <- dat_expanded %>% # applied to expanded data
    mutate(
      !!output_var_0 :=
        ...
        (coefs[[ses_var]] %||% 0) * 0 + 
        coefs["H1GI1Y"] * H1GI1Y +
        (coefs[[paste0(ses_var, ":H1GI1Y")]]) * H1GI1Y * 0
    )
  ...
  dat_expanded <- dat_expanded %>% # applied to expanded data
    mutate(
      !!output_var_1 :=
        ...
        coefs[[ses_var]] + 
        coefs["H1GI1Y"] * H1GI1Y +
        (coefs[[paste0(ses_var, ":H1GI1Y")]])  * H1GI1Y 
    )
...
for (wave in names(waves)) {
  for (ses_var in ses_vars) {  # NEW LINE: Nested loop over each ses variable
    ...
    saveRDS(results, file = paste0("/home/thom1336/healthdis/data/sensitivity/item/ses/bootstrap_results_", wave, "_", ses_var, ".rds"))
   # saveRDS(boot_results, file = paste0("/home/thom1336/healthdis/data/sensitivity/item/ses/boot_object_", wave, "_", ses_var, ".rds"))
