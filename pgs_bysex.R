...
dat_pheno_pgs_W2_male <- dat_pheno_pgs_W2 %>% dplyr::filter(sex == 0)
dat_pheno_pgs_W2_female <- dat_pheno_pgs_W2 %>% dplyr::filter(sex == 1)

dat_pheno_pgs_W4_male <- dat_pheno_pgs_W4 %>% dplyr::filter(sex == 0)
dat_pheno_pgs_W4_female <- dat_pheno_pgs_W4 %>% dplyr::filter(sex == 1)
...
for(sex in c("male", "female")){
  ...
  waves <- list(
    W2 = list(data = get(paste0("dat_pheno_pgs_W2_", sex)), outcome = "CESD_W2"),
    W4 = list(data = get(paste0("dat_pheno_pgs_W4_", sex)), outcome = "CESD_W4")
  )
...
for (wave in names(waves)) {
  for (pgs_var in pgs_vars) {  # loop over each pgs variable
    ...
    saveRDS(results, file = paste0("/home/thom1336/healthdis/data/sensitivity/sex/confound/pgs/bootstrap_results_ALL_", wave, sex, "_", pgs_var, ".rds"))
